...
class ClientUserViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if user != request.user:  # Проверяем, является ли пользователь, отправивший запрос, тем же пользователем
            return Response({'error': 'Вы можете изменять только свои собственные данные'},
...
def send_verification_code(request):
    if request.method == 'POST':
        ...
        # Если токен уже активен, возвращаем ошибку
        if verification_code.is_active and timezone.now() - verification_code.created_at > timezone.timedelta(
                minutes=2):
